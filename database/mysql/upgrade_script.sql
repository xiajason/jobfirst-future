...
-- 为现有表添加加密字段
ALTER TABLE users ADD COLUMN IF NOT EXISTS email_encrypted BLOB COMMENT '加密邮箱';
ALTER TABLE users ADD COLUMN IF NOT EXISTS phone_encrypted BLOB COMMENT '加密电话';
ALTER TABLE users ADD COLUMN IF NOT EXISTS first_name_encrypted BLOB COMMENT '加密名字';
ALTER TABLE users ADD COLUMN IF NOT EXISTS last_name_encrypted BLOB COMMENT '加密姓氏';

-- 为user_profiles表添加加密字段
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS date_of_birth_encrypted BLOB COMMENT '加密出生日期';
ALTER TABLE user_profiles ADD COLUMN IF NOT EXISTS location_encrypted BLOB COMMENT '加密位置信息';

-- 为files表添加加密字段
ALTER TABLE files ADD COLUMN IF NOT EXISTS file_path_encrypted BLOB COMMENT '加密文件路径';
ALTER TABLE files ADD COLUMN IF NOT EXISTS original_filename_encrypted BLOB COMMENT '加密原始文件名';
