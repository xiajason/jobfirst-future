-- Template Service 升级脚本
-- 添加新的字段支持评分和使用统计功能

-- 添加新字段到templates表
ALTER TABLE templates 
ADD COLUMN preview TEXT COMMENT '预览内容',
ADD COLUMN usage_count INT DEFAULT 0 COMMENT '使用次数',
ADD COLUMN rating DECIMAL(3,2) DEFAULT 0.00 COMMENT '评分(0-5)';
...
UPDATE templates SET 
    usage_count = 0,
    rating = 0.00,
    preview = SUBSTRING(content, 1, 200)
...
-- 插入一些示例模板数据（如果表为空）
INSERT IGNORE INTO templates (name, category, description, content, variables, preview, usage_count, rating, is_active, created_by, created_at, updated_at) VALUES
