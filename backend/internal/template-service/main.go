...
func main() {
	// 从环境变量获取端口，默认为8085
	port := os.Getenv("TEMPLATE_SERVICE_PORT")
	if port == "" {
		port = "8085"
	}
	...
	// 注册到Consul
	registerToConsul("template-service", "127.0.0.1", portInt)

	// 启动服务器
	log.Printf("Starting Template Service with jobfirst-core on 0.0.0.0:%s", port)
	if enhancedService != nil {
