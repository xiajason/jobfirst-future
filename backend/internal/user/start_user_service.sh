...
SERVICE_PORT="7530"
SERVICE_DIR="backend/internal/user"
SERVICE_BINARY="user-service"
...
start_service() {
    ...
    if go build -o "$SERVICE_BINARY" .; then
        log_success "$SERVICE_NAME 编译成功"
    else
    ...
    # 启动服务并重定向输出
    nohup ./"$SERVICE_BINARY" > "$LOG_FILE" 2>&1 &
    local service_pid=$!
