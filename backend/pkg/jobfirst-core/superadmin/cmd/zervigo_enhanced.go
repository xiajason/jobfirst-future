...
func calculateOverallHealth(startupStatus *system.StartupStatus, teamStatus *system.TeamStatus, userStatus *system.UserStatus) float64 {
	...
	healthScore += startupScore * 0.3
	totalChecks++

	// 团队状态检查 (权重: 35%)
	if teamStatus != nil {
		teamScore := getStatusScore(teamStatus.OverallStatus)
		healthScore += teamScore * 0.35
		totalChecks++
	}
	...
	if userStatus != nil {
		...
		healthScore += userScore * 0.35
		totalChecks++
	}

	return healthScore / float64(totalChecks)
}
...
func showHelp() {
	...
	fmt.Println()
	fmt.Println("报告文件: zervigo_report.json")
}
